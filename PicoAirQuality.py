...
from board import GP2, GP3, GP4, GP6, GP7, GP12, GP13, GP15, GP20
...
from math import ceil, trunc, floor, pow, e
...
class KitronikButton:
    def __init__(self):
        self.buttonA = DigitalInOut(GP12)
        self.buttonA.direction = Direction.INPUT
        self.buttonA.pull = Pull.DOWN
        self.buttonB = DigitalInOut(GP13)
        self.buttonB.direction = Direction.INPUT
        self.buttonB.pull = Pull.DOWN
...
class KitronikOutputControl:
    def __init__(self):
        ...
        # Frequency same as MicroPython default
        self.highPwr_3 = DigitalInOut(GP3)
        self.highPwr_15 = DigitalInOut(GP15)
        self.highPwr_3.direction = Direction.OUTPUT
        self.highPwr_15.direction = Direction.OUTPUT
        ...
        self.servo = StateMachine(pServo.assembled, frequency=1_000_000, first_set_pin=GP2, **pServo.pio_kwargs)
    ...
    def servoToPeriod(self, period):
        ...
        self.servo.background_write(memoryview(array('HH', [20_000 - period, period])).cast('L'))

    # Functions to turn on/off the high power outputs
    # Enter the pin number, either '3' or '15'
    def highPowerOn(self, pin):
        if pin == 3:
            self.highPwr_3.value = True
        elif pin == 15:
            self.highPwr_15.value = True

    def highPowerOff(self, pin):
        if pin == 3:
            self.highPwr_3.value = False
        elif pin == 15:
            self.highPwr_15.value = False
...
class KitronikDataLogger:
    ...
    def nameColumnHeadings(self, field1="", field2="", field3="", field4="", field5="", field6="", field7="", field8="", field9="", field10=""):
        ...
        if field2 != "":
            dataHeadings += field2 + self.SEPARATOR
        if field3 != "":
            dataHeadings += field3 + self.SEPARATOR
        if field4 != "":
            dataHeadings += field4 + self.SEPARATOR
        if field5 != "":
            dataHeadings += field5 + self.SEPARATOR
        if field6 != "":
            dataHeadings += field6 + self.SEPARATOR
        if field7 != "":
            dataHeadings += field7 + self.SEPARATOR
        if field8 != "":
            dataHeadings += field8 + self.SEPARATOR
        if field9 != "":
            dataHeadings += field9 + self.SEPARATOR
        if field10 != "":
            dataHeadings += field10 + self.SEPARATOR
    ...
    def storeDataEntry(self, field1="", field2="", field3="", field4="", field5="", field6="", field7="", field8="", field9="", field10=""):
        ...
        if field2 != "":
            dataEntry += field2 + self.SEPARATOR
        if field3 != "":
            dataEntry += field3 + self.SEPARATOR
        if field4 != "":
            dataEntry += field4 + self.SEPARATOR
        if field5 != "":
            dataEntry += field5 + self.SEPARATOR
        if field6 != "":
            dataEntry += field6 + self.SEPARATOR
        if field7 != "":
            dataEntry += field7 + self.SEPARATOR
        if field8 != "":
            dataEntry += field8 + self.SEPARATOR
        if field9 != "":
            dataEntry += field9 + self.SEPARATOR
        if field10 != "":
            dataEntry += field10 + self.SEPARATOR
...
class KitronikBuzzer:
    ...
    def __init__(self):
        self.buzzer = PWMOut(GP4, variable_frequency=True)
...
class KitronikZIPLEDs:
    ...
    def __init__(self, num_zip_leds):
        ...
        self.ZIPLEDs = KitronikZIPLEDsPIO(GP20, num_zip_leds, brightness=0.2, auto_write=False)
    ...
    def setLED(self, whichLED, whichColour):
        ...
        elif whichLED > self.num_zip_leds:
            raise Exception("INVALID LED:", whichLED, " specified")
        else:
...
class KitronikRTC:
    ...
    def setDate(self, day, month, year):
        ...
        self.rtc.datetime = struct_time((year, month, day, self.hour, self.minute, self.second, self.weekday, -1, -1))

    # Set the time on the Pico RTC
    def setTime(self, hour, minute, second):
        self.getDateTime()
        self.rtc.datetime = struct_time((self.year, self.month, self.day, hour, minute, second, self.weekday, -1, -1))
    ...
    def getDateTime(self):
        ...
        self.hour = newDateTime[3]
        self.minute = newDateTime[4]
        self.second = newDateTime[5]
        self.weekday = newDateTime[6]
    ...
    def setAlarm(self, hour, minute, alarmRepeat=False, hourPeriod=0, minutePeriod=0):
        self.alarmHour = ceil(hour)
        self.alarmMinute = ceil(minute)
        self.alarmRepeat = alarmRepeat
        
        if alarmRepeat:
            self.hourPeriod = ceil(hourPeriod)
            self.minutePeriod = ceil(minutePeriod)
        self.alarmSet = True
...
# Singleton to access the I2C connection
class KitronikI2C(object):
    def __new__(cls, *args, **kwargs):
        # Check if singleton instance already exists
        if not hasattr(cls, 'instance'):
            # Create the singleton instance
            cls.instance = super(KitronikI2C, cls).__new__(cls, *args, **kwargs)
            # Create the I2C connection
            cls.i2c = I2C(GP7, GP6, frequency=100_000)
...
class KitronikBME688:
    ...
    def getUInt8(self, reg):
        ...
        if self.i2c.try_lock():
            self.i2c.writeto_then_readfrom(self.CHIP_ADDRESS, outBuf, inBuf)
            self.i2c.unlock()
            return int.from_bytes(inBuf, "big")
        else:
            raise Exception("Could not get I2C lock")
    
    def writeToMem(self, reg, val):
        outBuf = bytearray(2)
        outBuf[0] = reg
        outBuf[1] = val
        if self.i2c.try_lock():
            self.i2c.writeto(self.CHIP_ADDRESS, outBuf)
            self.i2c.unlock()
        else:
            raise Exception("Could not get I2C lock")
    ...
    def setupGasSensor(self, targetTemp=300, heatDuration=180):
        ...
        self.writeToMem(0x5A, self.intConvertGasTargetTemp(self.tAmbient, targetTemp))   # res_wait_0 register - heater step 0
        ...
        self.writeToMem(0x64, codedDuration)     # gas_wait_0 register - heater step 0
        ...
        self.writeToMem(self.CTRL_GAS_1, (0x00 | gasEnable))   # Select heater step 0
    ...
    def measureData(self):
        ...
        self.writeToMem(self.CTRL_MEAS, 0x01 | oSampleTP)
        ...
        self.measTime = monotonic()  # Capture latest measurement time (ms since Pico powered on)
    ...
    def calcBaselines(self, forcedRun=False):
        ...
        try: # Look for a 'baselines.txt' file existing - if it does, take the baseline values from there (unless 'forcedRun' is set to True)
            ...
            while (burnInReadings < 60):               # Measure data and continue summing gas resistance until 60 readings have been taken
                progress = trunc((burnInReadings / 60) * 100)
                self.screen.clear()
    ...
    def calcAirQuality(self):
        ...
        # eCO2 Calculations
        self.eCO2Value = 250 * pow(e, (0.012 * self.iaqScore))      # Exponential curve equation to calculate the eCO2 from an iaqScore input
...
class KitronikOLED(FrameBuffer):
    # Write commands to the OLED controller
    def write_cmd(self, cmd):
        self.outBuf[0] = 0x80  # Co=1, D/C#=0
        self.outBuf[1] = cmd
        if self.i2c.try_lock():
            self.i2c.writeto(self.CHIP_ADDRESS, self.outBuf)
            self.i2c.unlock()
        else:
            raise Exception("Could not get I2C lock")
    ...
    def write_data(self, buf):
        # Co=0, D/C#=1
        self.write_list = bytearray([0x40]) + bytearray(buf)
        if self.i2c.try_lock():
            self.i2c.writeto(self.CHIP_ADDRESS, self.write_list)
            self.i2c.unlock()
        else:
            raise Exception("Could not get I2C lock")
    ...
    def __init__(self, i2cAddr=0x3C):
        self.CHIP_ADDRESS = i2cAddr # I2C address as determined by hardware configuration
    ...
    def displayText(self, text, line, x_offset=0):
        ...
        super().text(text, x_offset, y, True)
    ...
    def plot(self, variable):
        ...
        for entry in range(entries):
            ...
            y = trunc(self.yPixelMin - (y * ((self.yPixelMin - self.yPixelMax) / (self.plotYMax - self.plotYMin))))
            if (x == 0):
                super().pixel(x, y, 1)
            else:
